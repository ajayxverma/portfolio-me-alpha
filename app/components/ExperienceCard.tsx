...
const ExperienceCard = () => {
  ...
  const TabHeaderItem = (data: ExperienceType, index: number) => {
    return (
      <li onClick={() => handleTabClick(index)}>
        ...
        <ul role="list" className=" py-1 text-gray-600 dark:text-gray-300">
          ...
          <li className="list-item list-disc">
            <span>
              ...
              achieving reduced latency & improved application performance. Designed and implemented
              database Schemas to support data retrieval & storage
            </span>
          </li>
          <li className="list-item list-disc">
            <span>
                ...
                <div className="relative -mt-1 h-full overflow-auto pt-7 pb-6 sm:-mr-3 sm:pr-3">
                  <ul className="flex h-full w-max  justify-center space-x-2 px-6 sm:w-full sm:flex-col sm:space-x-0 sm:space-y-6 sm:px-8">
                    {experienceData.map((tab, index) => {
                      return <TabHeaderItem key={index} {...tab} />;
                    })}
                  </ul>
                </div>
              </div>
